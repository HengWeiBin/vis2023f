...
function _publi(FileAttachment){return(
...
function _public_columnKey(publi){return(
Object.keys(publi[0])[4]
)}
...
function _public_Column(publi,public_columnKey){return(
publi.map(row => row[public_columnKey])
)}

function _artist_uniqueValues(artist_Column){return(
...
function _artist_counts(artist_uniqueValues,artist_Column){return(
artist_uniqueValues.map(val => ({
  value: val,
  count: artist_Column.filter(v => v === val).length
}))
)}

function _public_counts(public_uniqueValues,public_Column){return(
public_uniqueValues.map(val => ({
  value: val,
  count: public_Column.filter(v => v === val).length
}))
)}

function _data(artist_counts,public_counts){return(
artist_counts.flatMap((item, index) => ([
  ...
  {
    value: item.value,
    count: public_counts[index].count,
    series: 'public'
  }
]))
)}
...
Inputs.checkbox(["artist", "public"], {label: "Choose datasets", value: ["artist", "public"]})
)}
...
Plot.plot({
  ...
  color: {
    domain: ['artist', 'public'],
    range: ['#DEB887', '#FFDEAD',],  // 你可以根據需要更改顏色
    legend: true
  },
  marks: [
...
function _selectedSVG(Inputs){return(
Inputs.checkbox(["artist", "public"], {label: "Choose datasets", value: ["artist", "public"]})
)}

function _chart(data,selectedSVG,d3)
{
  // 定義邊界大小，以及圖形的寬度和高度
  ...
  // 根據選擇的系列過濾數據
  const filteredData = data.filter(d => selectedSVG.includes(d.series));
...
md`### SVG 繪圖加入 transition`
)}

function _chart1(data,selectedSVG,d3)
{
  // 定義邊界大小，以及圖形的寬度和高度
  const margin = {top: 20, right: 30, bottom: 30, left: 40};
  const width = 500 - margin.left - margin.right;
...
md`### SVG 繪圖加入陰影、hover效果`
)}

function _chart2(data,selectedSVG,d3)
{
  // 定義邊界大小，以及圖形的寬度和高度
  const margin = {top: 20, right: 30, bottom: 30, left: 40};
  const width = 500 - margin.left - margin.right;
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("publi")).define("publi", ["FileAttachment"], _publi);
  main.variable(observer("artist_columnKey")).define("artist_columnKey", ["artist"], _artist_columnKey);
  main.variable(observer("public_columnKey")).define("public_columnKey", ["publi"], _public_columnKey);
  main.variable(observer("artist_Column")).define("artist_Column", ["artist","artist_columnKey"], _artist_Column);
  main.variable(observer("public_Column")).define("public_Column", ["publi","public_columnKey"], _public_Column);
  main.variable(observer("artist_uniqueValues")).define("artist_uniqueValues", ["artist_Column"], _artist_uniqueValues);
  main.variable(observer("public_uniqueValues")).define("public_uniqueValues", ["public_Column"], _public_uniqueValues);
  main.variable(observer("artist_counts")).define("artist_counts", ["artist_uniqueValues","artist_Column"], _artist_counts);
  main.variable(observer("public_counts")).define("public_counts", ["public_uniqueValues","public_Column"], _public_counts);
  main.variable(observer("data")).define("data", ["artist_counts","public_counts"], _data);
  ...
  main.variable(observer("viewof selectedSVG")).define("viewof selectedSVG", ["Inputs"], _selectedSVG);
  main.variable(observer("selectedSVG")).define("selectedSVG", ["Generators", "viewof selectedSVG"], (G, _) => G.input(_));
  main.variable(observer("chart")).define("chart", ["data","selectedSVG","d3"], _chart);
  main.variable(observer()).define(["md"], _19);
  main.variable(observer("chart1")).define("chart1", ["data","selectedSVG","d3"], _chart1);
  main.variable(observer()).define(["md"], _21);
  main.variable(observer("chart2")).define("chart2", ["data","selectedSVG","d3"], _chart2);
  return main;
}
